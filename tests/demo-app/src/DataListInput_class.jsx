...
class DataListInput extends React.Component {
   ...
   * @returns {Array}
   */
  matching = (currentInput, items, match) =>
    items.filter(item => {
  ...
  indexOfMatch = (currentInput, item) =>
    item.label.toUpperCase().indexOf(currentInput.toUpperCase());

  indexOfItem = (item, items) =>
  ...
  debouncedMatchingUpdateStep = currentInput => {
    ...
    const updateMatchingItems = () => {
      ...
      const index = showDragIndex
        ? this.indexOfItem(lastValidItem, displayableItems)
        : 0;
    ...
    if (debounceTime <= 0) {
      ...
      this.inputHappenedTimeout = window.setTimeout(
        updateMatchingItems,
        debounceTime
      );
    }
  ...
  renderMatchingLabel = (currentInput, item, indexOfMatch) => (
    <>
      ...
      <strong>{item.label.substr(indexOfMatch, currentInput.length)}</strong>
        ...
        return (
          ...
          >
            {this.renderItemLabel(
              currentInput,
              item,
              this.indexOfMatch(currentInput, item)
            )}
  ...
  render() {
    ...
    if (reachedRequiredLength && isMatchingDebounced) {
      renderedResults = this.renderLoader(
        debounceLoader,
        itemClassName,
        dropdownClassName
      );
    } else if (reachedRequiredLength && visible) {
      renderedResults = this.renderItems(
        currentInput,
        matchingItems,
        focusIndex,
        activeItemClassName,
